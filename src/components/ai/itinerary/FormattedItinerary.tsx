...
export default function FormattedItinerary({ 
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden">
      {/* Header */}
      <div className="bg-filipino-teal px-3 py-4 sm:px-6 sm:py-5 text-white">
        <h1 className="text-lg sm:text-xl md:text-2xl font-bold break-words">{finalTitle}</h1>
        <div className="flex flex-wrap items-center gap-2 mt-2">
          <div className="flex items-center">
            <Calendar className="h-3.5 w-3.5 sm:h-4 sm:w-4 mr-1 flex-shrink-0" />
            <span className="text-xs sm:text-sm">{finalDate}</span>
          </div>
          {subtitle && (
            <div className="flex items-center">
              <span className="mx-1 hidden sm:inline">•</span>
              <span className="text-xs sm:text-sm">{finalSubtitle}</span>
            </div>
          )}
        </div>
        {description && <p className="mt-2 text-white/90 text-xs sm:text-sm line-clamp-3">{description}</p>}
      </div>
      ...
      <div className="p-2 sm:p-4 md:p-6">
        {finalSections.map((section, sectionIndex) => (
          ...
          >
            <div className="bg-filipino-teal/10 px-3 py-2 border-b">
              <CollapsibleTrigger className="flex items-center justify-between w-full text-left">
                <div className="pr-2">
                  <h2 className="text-sm sm:text-base font-semibold text-filipino-deepTeal line-clamp-1">{section.title}</h2>
                  {section.date && (
                    <span className="text-xs text-muted-foreground block truncate">{section.date}</span>
                  )}
                </div>
                {openSections[sectionIndex] ? (
                  <ChevronUp className="h-4 w-4 text-filipino-teal flex-shrink-0" />
                ) : (
                  <ChevronDown className="h-4 w-4 text-filipino-teal flex-shrink-0" />
                )}
              </CollapsibleTrigger>
            </div>
                ...
                <div className="space-y-3 sm:space-y-4">
                  {section.places.map((place, placeIndex) => (
                    <Card key={placeIndex} className="overflow-hidden hover:shadow-md transition-shadow">
                      <div className="flex flex-col md:grid md:grid-cols-3 gap-2 sm:gap-3">
                        <div className="relative h-36 sm:h-44 md:h-full">
                          <img 
                            src={place.imageUrl} 
                            alt={place.name}
                        ...
                        <div className="p-2 sm:p-3 md:col-span-2">
                          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-2">
                            <h3 className="font-bold text-sm sm:text-base line-clamp-2 pr-1">{place.name}</h3>
                          ...
                          <p className="mt-1.5 sm:mt-2 text-xs sm:text-sm text-gray-600 line-clamp-3 sm:line-clamp-4">
                            {place.description}
                          </p>
                          
                          {place.entranceFee && (
                            <div className="mt-2">
                              <span className="inline-block text-xs bg-filipino-warmOchre/10 text-filipino-warmOchre px-2 py-0.5 rounded">
                          ...
                          <div className="mt-2 sm:mt-3 flex flex-wrap gap-2">
                            <Button 
                              ...
                              className="bg-filipino-terracotta hover:bg-filipino-terracotta/90 text-xs sm:text-sm h-7 sm:h-8"
                            >
        ...
        <div className="mt-4 sm:mt-6 border-t pt-3 sm:pt-4">
          <h2 className="text-base sm:text-lg font-semibold text-filipino-deepTeal mb-2 sm:mb-3">Additional Information</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
            <Card className="p-2 sm:p-3">
              <h3 className="font-bold text-xs sm:text-sm mb-1.5 sm:mb-2">Transportation</h3>
            ...
            <Card className="p-2 sm:p-3">
              <h3 className="font-bold text-xs sm:text-sm mb-1.5 sm:mb-2">Practical Tips</h3>
              <ul className="list-disc pl-4 sm:pl-5 space-y-1 text-xs">
      ...
      <Dialog open={showMap} onOpenChange={setShowMap}>
        <DialogContent className="sm:max-w-md max-w-[calc(100%-2rem)] p-0 overflow-hidden">
          <DialogHeader className="p-2 sm:p-3 pb-1 sm:pb-2">
            <DialogTitle className="text-sm sm:text-base break-words pr-6">
              {selectedPlace?.name}
            </DialogTitle>
          </DialogHeader>
